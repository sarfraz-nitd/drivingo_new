<?php
    ...
    $query1 = "SELECT * FROM comments WHERE school_id = '".$schoolId."'";
    if($query1_run = mysqli_query($mysqli, $query1)){
            ...
            while($row = mysqli_fetch_assoc($query1_run)){
                ...
                if($row['user_type'] == 'user'){
                    if($row['user_table'] == 'g_users'){
                        $comment_first_name = get_field($row['user_table'], 'first_name', $row['user_id']);
                        $comment_last_name = get_field($row['user_table'], 'last_name', $row['user_id']);
                        $comment_picture[$comment_count] = get_field($row['user_table'], 'picture', $row['user_id']);
                        $comment_name[$comment_count] = $comment_first_name.' '.$comment_last_name;
                    } else if($row['user_table'] == 'fb_users'){
                        $comment_name[$comment_count] = get_field($row['user_table'], 'name', $row['user_id']);
                        $comment_picture[$comment_count] = get_field($row['user_table'], 'image', $row['user_id']);
                    } else if($row['user_table'] == 'users'){
                        $comment_name[$comment_count] = get_field($row['user_table'], 'name', $row['user_id']);
                        $comment_picture[$comment_count] = 'uploads/'.get_field($row['user_table'], 'email', $row['user_id']).'/cover_photo.jpg';
                    }
                    ...
                    if($row['user_table'] == 'g_users'){
                        ...
                        $comment_name[$comment_count] = get_field($row['user_table'], 'owners_name', $row['user_id']);
                        $comment_picture[$comment_count] = get_field($row['user_table'], 'image', $row['user_id']);
                    } else {
                        $comment_name[$comment_count] = get_field($row['user_table'], 'name', $row['user_id']);
                        $comment_picture[$comment_count] = 'uploads/'.get_field($row['user_table'], 'email', $row['user_id']).'/cover_photo.jpg';
                ...
                $comment_text[$comment_count] = get_field('comments', 'comment', $row['id']);
                $comment_time[$comment_count] = get_field('comments', 'time', $row['id']);   
    ...
    function get_field($table_name, $field, $id){
        global $mysqli;
        $query = "SELECT $field FROM $table_name WHERE id = $id";
        if($query_run = mysqli_query($mysqli, $query)){
    ...
    function auth_comment($sid, $uid, $stable, $utable){

        global $mysqli;
        $flag = false;
        $query = "SELECT * FROM userPayment WHERE school_id = ".$sid." AND user_id = ".$uid." AND school_table = '".$stable."' AND user_table = '".$utable."'";
        if($query_run = mysqli_query($mysqli, $query)){
            if(mysqli_num_rows($query_run) > 0){
                $flag = true;
            } 
            ...
            $(document).ready(function(){
                ...
                comment_input.addEventListener('keydown', function(event){
                    if(event.keyCode == 13){
                        ...
                        var school_table = "<?php echo $table; ?>";
                        var user_id = <?php echo $_SESSION['loggedin']; ?>;
                        var user_type = "<?php echo $_SESSION['user_type']; ?>";
                        var user_table = "<?php echo $_SESSION['table']; ?>";
                        var formData = new FormData();
                        formData.append('comment', comment);
                        formData.append('school_id', school_id);
                        formData.append('user_id', user_id);
                        formData.append ('school_table', school_table);
                        formData.append('user_type', user_type);
                        formData.append('user_table', user_table);
                        ...
                        $.ajax({
                            ...
                            success : function(data){
                                ...
                                var result = "";
                                for(var i=comment_data.length-1;i>=0;i--){
                                    result += '<div class="row">' +
                                                '<div class="col s1">' +
                                                    '<div class="comment_image">' +
                                                        '<img src="' + comment_data[i].picture + '" height="60" width="60" style="box-shadow: 0 2px 6px rgba(0, 0, 0, 0.3);" class="circle" />' +
                                                    '</div>' +
                                                '</div>' +
                                                '<div class="col s11">' +
                                                  '<div class="card gradient">' +
                                                    '<div class="card-content white-text">' +
                                                      '<span class="comment_name">' + comment_data[i].name + '</span><span class="comment_dot">.</span><span class="comment_time">' + comment_data[i].time + '</span>' +
            ...
            <div class="row">
                ...
                <div class="col s12 mm9 l12" onmouseover="hideName('#image-header');">
                  ...
                  <div id="comments" class="section scrollspy move-right">
                     ...
                     <?php if(auth_comment($schoolId, $_SESSION['loggedin'], $table, $_SESSION['table'])) { ?>
                         <div class="row">
                            <div class="input-field col s12">
                              <input id="comment_input" type="text" class="validate" placeholder="Post a comment...">
                            </div> 
                         </div>
                    <?php } ?>
                     <div id="comment_section" class="comment-section">

                        <?php for($i=$comment_count-1; $i>=0; $i--) { ?>

                          <div class="row">
                            <div class="col s1">
                                <div class="comment_image">
                                    <img src="<?php echo $comment_picture[$i]; ?>" height="60" width="60" style="box-shadow: 0 2px 6px rgba(0, 0, 0, 0.3);" class="circle" />
                                </div>
                            </div>
                            <div class="col s11">
                              <div class="card gradient">
                                <div class="card-content white-text">
                                  <span class="comment_name"><?php echo $comment_name[$i]; ?></span><span class="comment_dot">.</span><span class="comment_time"><?php echo $comment_time[$i]; ?></span>
                                  <p><?php echo $comment_text[$i]; ?></p>
                                </div>
                              </div>
                            </div>
                          </div>
